...
# ✅ 计算 CKA 相似性（基于 Gram 矩阵）
def compute_cka(H1, H2):
    """
    计算 CKA（Centered Kernel Alignment），衡量两个层之间的相似性
    H1, H2: 形状为 (batch_size, hidden_dim) 的张量
    """
    H1 = H1 - H1.mean(dim=0, keepdim=True)  # 中心化
    H2 = H2 - H2.mean(dim=0, keepdim=True)  # 中心化

    K1 = H1 @ H1.T  # Gram 矩阵
    K2 = H2 @ H2.T  # Gram 矩阵

    norm1 = torch.norm(K1, p='fro')  # Frobenius 范数
    norm2 = torch.norm(K2, p='fro')  # Frobenius 范数

    return (torch.sum(K1 * K2) / (norm1 * norm2)).item()
...
# ✅ CKA 归一化（防止极端 1.0 影响剪枝）
cka_values = (cka_values - np.min(cka_values)) / (np.max(cka_values) - np.min(cka_values))

# ✅ 计算 CKA 修正 ESD 剪枝比例
alpha = 0.1  # CKA 影响力（可以调整）
